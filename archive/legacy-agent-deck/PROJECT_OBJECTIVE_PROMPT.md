...
```text
https://github.com/Dimillian/AgentDeck.git
...
Use AgentDeck as the primary foundation.

The owner likes AgentDeck's interface: the transparent desktop feel, dark glass styling, side panel, layout density, and overall interaction model. The next project should begin from AgentDeck rather than trying to recreate that look from scratch. The goal is to initialize the root project using AgentDeck as the real baseline, then gradually modify it into the desired multi-agent session dashboard.
...
The previous RunDeck implementation over-invested in a custom product architecture before the desired user experience was stable. It preserved too much backend/domain planning while trying to bolt a AgentDeck-like shell onto an existing scaffold. That produced friction and drift:

- The app did not feel like AgentDeck even after shell work.
...
1. Treat `sources/AgentDeck` as the app foundation.
2. Initialize the project root from AgentDeck, not from the archived RunDeck app.
3. Keep the AgentDeck visual system intact at first: transparency, sidebar, layout, shell density, terminal/review surfaces, and dark desktop feel.
4. Do not immediately rename everything, restructure everything, or replace the shell.
5. First get the AgentDeck-based app running from the root.
...
The tile grid should feel native to the AgentDeck shell. It should not look like a generic card board pasted into the app. Preserve AgentDeck's visual vocabulary and adapt the tiles to it.
...
- Copy or scaffold from `sources/AgentDeck` into the root.
- Preserve `sources/` as reference material.
- Do not delete `archived-rundeck-project`.
- Get the AgentDeck-derived app install/build/dev workflow working from the root.
...
- Confirm the sidebar and main layout match AgentDeck before changing product behavior.
...
- Design a AgentDeck-native tile grid.
...
- Do not flatten AgentDeck's visual style into a basic Vite/Tauri scaffold.
...
looks and feels like AgentDeck,
...
git -C sources/AgentDeck status --short
git -C sources/agtx status --short
```

Then inspect AgentDeck:

```powershell
Get-Content -Raw sources/AgentDeck/package.json
Get-Content -Raw sources/AgentDeck/src-tauri/tauri.conf.json
Get-ChildItem sources/AgentDeck/src -Force
Get-ChildItem sources/AgentDeck/src/styles -Force
```
...
After that, initialize the root from AgentDeck and get it running.
