...
const allowedOrchestratorProviders = new Set([
  "openai",
  "codex",
  "anthropic",
  "ollama",
...
const DEFAULT_ORCHESTRATOR_BASE_URL = "https://api.openai.com/v1";
...
function normalizeOrchestratorConfig(
  config: AppSettings["orchestratorConfig"],
): AppSettings["orchestratorConfig"] {
  if (!config) {
    return null;
  }
  const provider = allowedOrchestratorProviders.has(config.provider)
    ? config.provider
    : "openai";
  const maxConcurrentAgents = Number.isFinite(config.maxConcurrentAgents)
    ? Math.min(12, Math.max(1, Math.round(config.maxConcurrentAgents)))
    : 3;
  ...
  return {
    ...
    baseUrl: isCodexProvider
      ? config.baseUrl?.trim() || ""
      : config.baseUrl?.trim() || DEFAULT_ORCHESTRATOR_BASE_URL,
    apiKey: isCodexProvider ? null : config.apiKey?.trim() || null,
    model: isCodexProvider
      ? config.model?.trim() || ""
      : config.model?.trim() || DEFAULT_ORCHESTRATOR_MODEL,
...
function normalizeAppSettings(settings: AppSettings): AppSettings {
  ...
  return {
    ...
    theme: allowedThemes.has(settings.theme) ? settings.theme : DEFAULT_THEME,
    ...
    selectedOpenAppId,
    orchestratorConfig: normalizeOrchestratorConfig(settings.orchestratorConfig),
  };
}
