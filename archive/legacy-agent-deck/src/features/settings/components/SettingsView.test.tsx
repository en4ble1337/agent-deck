...
vi.mock("@services/tauri", async () => {
  ...
  return {
    ...
    orchestratorTestConnection: vi.fn(),
    runCodexDeviceLogin: vi.fn(),
  };
});
...
const orchestratorTestConnectionMock = vi.mocked(orchestratorTestConnection);
const runCodexDeviceLoginMock = vi.mocked(runCodexDeviceLogin);
...
orchestratorTestConnectionMock.mockResolvedValue({
  message: { role: "assistant", content: "OK" },
  toolCalls: [],
  finishReason: "stop",
  error: null,
});
runCodexDeviceLoginMock.mockResolvedValue({
...
const renderDisplaySection = (
  ...
  render(<SettingsView {...props} initialSection={options.initialSection} />);
  if (!options.skipDisplayClick) {
...
describe("SettingsView Orchestrator", () => {
  it("saves orchestrator connection settings", async () => {
    const onUpdateAppSettings = vi.fn().mockResolvedValue(undefined);
    renderDisplaySection({
      initialSection: "orchestrator",
      skipDisplayClick: true,
      onUpdateAppSettings,
    });
  ...
  it("tests orchestrator connection from the draft config", async () => {
    renderDisplaySection({
      initialSection: "orchestrator",
      skipDisplayClick: true,
    });
    ...
    await waitFor(() =>
      expect(orchestratorTestConnectionMock).toHaveBeenCalledWith(
        expect.objectContaining({
          apiKey: "sk-test",
          model: "gpt-4o-mini",
        }),
  ...
  it("saves Codex subscription mode without an API key", async () => {
    const onUpdateAppSettings = vi.fn().mockResolvedValue(undefined);
    renderDisplaySection({
      initialSection: "orchestrator",
      skipDisplayClick: true,
  ...
  it("starts global Codex device auth when no workspace is connected", async () => {
    const { onUpdateAppSettings } = renderDisplaySection({
      initialSection: "orchestrator",
      skipDisplayClick: true,
    });
    ...
    await waitFor(() => expect(runCodexDeviceLoginMock).toHaveBeenCalled());
    expect(onUpdateAppSettings).toHaveBeenCalledWith(
      expect.objectContaining({
        orchestratorConfig: expect.objectContaining({
          provider: "codex",
    ...
    expect(await screen.findByText("Opened Codex auth. Enter code ABCD-1234")).toBeTruthy();
  });

  it("treats global Codex device auth as complete when already signed in", async () => {
    openUrlMock.mockClear();
    runCodexDeviceLoginMock.mockResolvedValueOnce({
    ...
    expect(await screen.findByText("Codex is already authorized")).toBeTruthy();
    expect(onUpdateAppSettings).toHaveBeenCalledWith(
      expect.objectContaining({
        orchestratorConfig: expect.objectContaining({
          provider: "codex",
