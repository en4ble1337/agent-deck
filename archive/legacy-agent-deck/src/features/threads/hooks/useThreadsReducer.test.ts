...
describe("threadReducer", () => {
  it("ensures thread with default name and active selection", () => {
    ...
    expect(threads[0].name).toBe("New Session");
  ...
  it("renames auto-generated thread on first user message", () => {
    const threads: ThreadSummary[] = [
      { id: "thread-1", name: "New Session", updatedAt: 1 },
    ];
  ...
  it("keeps an agent message when an older thread preview arrives", () => {
    const base: ThreadState = {
      ...initialState,
      lastAgentMessageByThread: {
        "thread-1": {
  ...
  it("accepts a newer thread preview after stale agent output", () => {
    const base: ThreadState = {
      ...initialState,
      lastAgentMessageByThread: {
        "thread-1": {
          text: "Older agent output",
          timestamp: 1000,
          source: "agent",
        },
      },
