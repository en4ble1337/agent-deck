...
const SESSION_DECK_LIMIT = 9;
const IDLE_SESSION_STALE_AFTER_MS = 60 * 60 * 1000;
const SESSION_BOARD_REFRESH_MS = 60 * 1000;
const SESSION_OUTPUT_PLACEHOLDER = "No recent output loaded yet.";
...
function isRecentIdleSession(session: SessionTile, now: number) {
  return !isActionableSession(session) && now - session.updatedAt <= IDLE_SESSION_STALE_AFTER_MS;
}

function normalizePreviewComparison(value: string) {
  return value
    .replace(/\s+/g, " ")
    .replace(/[.…]+$/g, "")
    .trim()
...
function isThreadPreviewTitleEcho(
  message: string,
  threadName: string,
  title: string,
) {
  const normalizedMessage = normalizePreviewComparison(message);
  const normalizedThreadName = normalizePreviewComparison(threadName);
  const normalizedTitle = normalizePreviewComparison(title);
  if (!normalizedMessage) {
    return true;
  }
  return (
    normalizedMessage === normalizedThreadName ||
    normalizedMessage === normalizedTitle ||
    normalizedMessage.startsWith(normalizedThreadName) ||
...
function sortSessionsForBoard(sessions: SessionTile[]) {
  return sessions.slice().sort((a, b) => {
    ...
    return getSessionBoardPriority(a) - getSessionBoardPriority(b);
  });
}
      ...
      return threads.map((thread) => {
        ...
        const preview =
          latestMessage && latest?.source !== "thread-preview"
            ? latestMessage
            : latestMessage && !isThreadPreviewTitleEcho(latestMessage, threadName, title)
              ? latestMessage
            : SESSION_OUTPUT_PLACEHOLDER;
...
export function HomeSessionDashboard({
  ...
  const [sessionBoardNow, setSessionBoardNow] = useState(() => Date.now());
  const outputLoadRequestedSessionIdsRef = useRef<Set<string>>(new Set());

  useEffect(() => {
    const refreshId = window.setInterval(() => {
      setSessionBoardNow(Date.now());
    }, SESSION_BOARD_REFRESH_MS);
  ...
  useEffect(() => {
    ...
    if (!workspaceOptions.some((workspace) => workspace.id === workspaceFilter)) {
  ...
  const deckSessions = useMemo(() => {
    const boardCandidates =
      sessionFilter === "all"
        ? boardSortedSessions.filter(
            (session) =>
              isActionableSession(session) ||
              isRecentIdleSession(session, sessionBoardNow),
          )
        : boardSortedSessions;
    ...
    const selectedSession = boardCandidates.find((session) => session.id === selectedId);
    if (!selectedSession) {
      return firstSessions;
    }
    return [
  ...
  }, [boardSortedSessions, selectedId, sessionBoardNow, sessionFilter]);

  useEffect(() => {
    if (!onLoadSessionOutput) {
      return;
    }
    deckSessions.forEach((session) => {
      if (session.preview !== SESSION_OUTPUT_PLACEHOLDER) {
        outputLoadRequestedSessionIdsRef.current.delete(session.id);
        return;
      }
      if (
        session.source === "real" &&
        session.threadId &&
        !outputLoadRequestedSessionIdsRef.current.has(session.id)
      ) {
        outputLoadRequestedSessionIdsRef.current.add(session.id);
        onLoadSessionOutput(session.workspaceId, session.threadId);
      }
    });
  }, [deckSessions, onLoadSessionOutput]);
  ...
  const emptyMessage =
    workspaces.length === 0
      ? "Add a workspace to start a session."
    ...
    <section className="session-dashboard" aria-labelledby="session-dashboard-title">
      <div className="session-dashboard-header">
        <div className="session-dashboard-heading">
          <div className="session-dashboard-kicker">Workspaces</div>
          <h1 id="session-dashboard-title">Terminal Mux</h1>
        </div>
