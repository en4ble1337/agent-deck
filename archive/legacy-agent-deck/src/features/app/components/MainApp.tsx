...
import { OrchestratorDrawer } from "@/features/orchestrator/components/OrchestratorDrawer";
...
const SIDEBAR_THEME_CYCLE: ThemePreference[] = ["dark", "dim", "light"];
...
export default function MainApp() {
  ...
  const handleCloseOrchestrator = useCallback(() => {
    setOrchestratorOpen(false);
  }, []);
  const orchestratorToggleNode = useMemo(
    () => (
      <button
        type="button"
        className={`session-dashboard-stat session-dashboard-stat--orchestrator${
          orchestratorOpen ? " is-active" : ""
        }`}
        onClick={handleToggleOrchestrator}
        aria-pressed={orchestratorOpen}
        title="Toggle Orchestrator (Ctrl+Shift+O)"
      >
        <span>AI</span>
        <span>orchestrator</span>
      </button>
    ),
    [handleToggleOrchestrator, orchestratorOpen],
  );
  const {
    threadListSortKey,
  ...
  const handleCycleTheme = useCallback(() => {
    const currentIndex = SIDEBAR_THEME_CYCLE.indexOf(appSettings.theme);
    const nextTheme =
      SIDEBAR_THEME_CYCLE[
        currentIndex >= 0 ? (currentIndex + 1) % SIDEBAR_THEME_CYCLE.length : 0
      ];
  ...
  const orchestratorSettingsWorkspace =
    activeWorkspace ?? workspaces.find((workspace) => workspace.connected) ?? null;
  const orchestratorSettingsAccount = orchestratorSettingsWorkspace
    ? accountByWorkspace[orchestratorSettingsWorkspace.id] ?? null
    : null;
  const handleSwitchOrchestratorAccount = useCallback(
    () => handleSwitchAccount(orchestratorSettingsWorkspace?.id ?? null),
    [handleSwitchAccount, orchestratorSettingsWorkspace?.id],
  );
  const handleCancelSwitchOrchestratorAccount = useCallback(
    () => handleCancelSwitchAccount(orchestratorSettingsWorkspace?.id ?? null),
    [handleCancelSwitchAccount, orchestratorSettingsWorkspace?.id],
  );
  ...
  const { appModalsProps, modalActions } = useMainAppModals({
    ...
    settings: {
      ...
      handleMobileConnectSuccess,
      dictationModel,
      activeWorkspace: orchestratorSettingsWorkspace,
      activeAccount: orchestratorSettingsAccount,
      accountSwitching,
      onSwitchCodexAccount: handleSwitchOrchestratorAccount,
      onCancelCodexAccountSwitch: handleCancelSwitchOrchestratorAccount,
    },
  });
  ...
  const layoutSurfaces = useMainAppLayoutSurfaces({
    ...
    onLoadHomeSessionOutput: (workspaceId, threadId) => {
      if (hasLocalThreadSnapshot(threadId)) {
        return;
      }
  ...
  return (
    <>
      ...
      <OrchestratorDrawer
        ...
        orchestratorConfig={appSettings.orchestratorConfig ?? null}
        isOpen={orchestratorOpen}
        onToggle={handleToggleOrchestrator}
        onClose={handleCloseOrchestrator}
        onOpenSettings={() => modalActions.openSettings("orchestrator")}
      />
    </>
  );
}
