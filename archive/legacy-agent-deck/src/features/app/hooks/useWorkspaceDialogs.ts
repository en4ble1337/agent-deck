...
export function useWorkspaceDialogs() {
  ...
  const confirmWorkspaceRemoval = useCallback(
    async (workspaces: WorkspaceInfo[], workspaceId: string) => {
      ...
      const detail =
        worktreeCount > 0
          ? `\n\nThis will also close ${worktreeCount} worktree agent${
              worktreeCount === 1 ? "" : "s"
            } and delete their worktree folder${worktreeCount === 1 ? "" : "s"} from disk.`
          : "";

      return ask(
        `Remove "${workspaceName}" from AgentDeck?\n\nThis unloads the workspace from the app. Project files stay on disk.${detail}`,
        {
  ...
  const confirmWorktreeRemoval = useCallback(
    async (workspaces: WorkspaceInfo[], workspaceId: string) => {
      ...
      return ask(
        `Remove "${workspaceName}" from AgentDeck?\n\nThis will close the agent, remove it from the app, and delete its worktree folder from disk.`,
        {
