...
type UseAccountSwitchingResult = {
  ...
  handleCancelSwitchAccount: (workspaceIdOverride?: string | null) => Promise<void>;
};
  ...
  const handleSwitchAccount = useCallback(
    async (workspaceIdOverride?: string | null) => {
      ...
      if (!workspaceId) {
        ...
        loginWorkspaceIdRef.current = null;
        try {
          const { authUrl } = await runCodexDeviceLogin();
          if (authUrl && !accountSwitchCanceledRef.current) {
  ...
  const handleCancelSwitchAccount = useCallback(
    async (workspaceIdOverride?: string | null) => {
      const targetWorkspaceId =
        loginWorkspaceIdRef.current ?? workspaceIdOverride ?? activeWorkspaceId;
      if (
        !targetWorkspaceId ||
