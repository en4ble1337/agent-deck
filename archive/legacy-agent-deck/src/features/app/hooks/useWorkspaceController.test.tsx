...
describe("useWorkspaceController dialogs", () => {
  ...
  it("confirms workspace removal and reports service errors", async () => {
    vi.mocked(listWorkspaces).mockResolvedValue([workspaceOne]);
    ...
    expect(options).toEqual(
      expect.objectContaining({ title: "Remove workspace failed", kind: "error" }),
    );
  });
  ...
  it("opens the in-app remote path prompt on mobile remote mode", async () => {
    ...
    await act(async () => {
      result.current.updateMobileRemoteWorkspacePathInput("/srv/agent-deck");
    });
    ...
    expect(isWorkspacePathDir).toHaveBeenCalledWith("/srv/agent-deck");
