...
export type OrchestratorSendResponse = {
  message: { role: string; content: string; toolCalls?: unknown[] } | null;
  toolCalls: {
    id: string;
    type: string;
...
export async function orchestratorSend(
  messages: { role: string; content: string; tool_call_id?: string }[],
  workspaceId?: string | null,
): Promise<OrchestratorSendResponse> {
  return invoke<OrchestratorSendResponse>("orchestrator_send", {
    messages,
    workspaceId: workspaceId ?? null,
  });
}

export async function orchestratorTestConnection(
  config: OrchestratorConfigInput,
  workspaceId?: string | null,
): Promise<OrchestratorSendResponse> {
  return invoke<OrchestratorSendResponse>("orchestrator_test_connection", {
    config,
    workspaceId: workspaceId ?? null,
  });
}
