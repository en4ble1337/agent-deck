...
Keep AgentDeck's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between AgentDeck settings and Codex runtime behavior
...
## AgentDeck Files To Update If Upstream Changes
...
- `src-tauri/src/bin/agent_deck_daemon/rpc/codex.rs`
...
- Keep AgentDeck scoped to the new key only (no legacy alias read/write).
- Keep AgentDeck writes aligned with upstream expectations.
...
- Reconcile AgentDeck guardrails when upstream changes.
...
- AgentDeck default `agents.max_depth` is `1`.
- AgentDeck currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
