# AgentDeck iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted AgentDeck backend over Tailscale.
...
1. Desktop AgentDeck hosts the daemon and executes Codex workflows.
2. iOS AgentDeck connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop AgentDeck is installed and able to run local workspaces.
...
1. Build daemon + daemonctl:
   - `cd src-tauri`
   - `cargo build --bin agent_deck_daemon --bin agent_deck_daemonctl`
2. Start daemon from CLI:
   - `./target/debug/agent_deck_daemonctl start`
3. Verify daemon status:
   - `./target/debug/agent_deck_daemonctl status`
