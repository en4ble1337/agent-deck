...
- Which app-server events AgentDeck supports right now.
- Which app-server requests AgentDeck sends right now.
- Where to look in AgentDeck to add support.
...
3. Compare Codex events vs AgentDeck routing.
4. Compare Codex client request methods vs AgentDeck outgoing request methods.
5. Compare Codex server request methods vs AgentDeck inbound request handling.
6. Update supported and missing lists below.

Related project skill:
- `.codex/skills/app-server-events-sync/SKILL.md`

## Where To Look In AgentDeck
...
- `src-tauri/src/bin/agent_deck_daemon.rs`

## Supported Notifications (Codex v2)

These are the current Codex v2 `ServerNotification` methods that AgentDeck
...
## Additional Stream Methods Handled In AgentDeck
...
- `codex/backgroundThread` (AgentDeck synthetic bridge event)
- `codex/connected` (AgentDeck synthetic bridge event)
...
## Supported Requests (AgentDeck -> App-Server, v2)

These are v2 request methods AgentDeck currently sends to Codex app-server:
...
Compared against Codex v2 request methods, AgentDeck currently does not send:
...
## Server Requests (App-Server -> AgentDeck, v2)
...
3. List AgentDeck routed methods:
...
4. List AgentDeck outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - AgentDeck generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - AgentDeck attempts `turn/steer` only when steer capability is enabled, the thread is processing, and an active turn id exists.
  - If `turn/steer` fails, AgentDeck does not fall back to `turn/start`; it clears stale processing/turn state when applicable, surfaces an error, and returns `steer_failed`.
