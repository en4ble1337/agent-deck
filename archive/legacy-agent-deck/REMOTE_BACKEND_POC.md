...
This fork includes a **proof-of-concept** daemon that runs AgentDeck's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin agent_deck_daemon -- \
  --listen 127.0.0.1:4732 \
  --data-dir "$HOME/.local/share/agent-deck-daemon" \
  --token "$TOKEN"
```
