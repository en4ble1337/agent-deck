...
fn build_clone_destination_path_sanitizes_and_uniquifies() {
    let temp_dir = std::env::temp_dir().join(format!("agent-deck-test-{}", Uuid::new_v4()));
...
#[test]
fn remove_workspace_deletes_child_directly_when_git_worktree_remove_fails() {
    run_async(async {
        let temp_dir = std::env::temp_dir().join(format!("agent-deck-test-{}", Uuid::new_v4()));
        let parent_repo_path = temp_dir.join("parent-repo");
        let child_path = temp_dir.join("worktrees").join("parent").join("feature-a");
        std::fs::create_dir_all(&parent_repo_path).expect("create parent repo path");
        std::fs::create_dir_all(&child_path).expect("create child path");
        std::fs::write(child_path.join("file.txt"), "content").expect("write child file");
        ...
        let child = WorkspaceEntry {
            id: "wt-git-delete-failed".to_string(),
            name: "feature-a".to_string(),
            path: child_path.to_string_lossy().to_string(),
            kind: WorkspaceKind::Worktree,
            parent_id: Some(parent.id.clone()),
        ...
        remove_workspace_core(
            ...
            |_root, args| {
                ...
                async move {
                    if command.starts_with("worktree remove") {
                        Err("fatal: failed to delete worktree: Permission denied".to_string())
                    } else {
                        Ok(())
                    }
                }
...
#[test]
fn remove_worktree_deletes_folder_directly_when_git_worktree_remove_fails() {
    run_async(async {
        let temp_dir = std::env::temp_dir().join(format!("agent-deck-test-{}", Uuid::new_v4()));
        let parent_repo_path = temp_dir.join("parent-repo");
        let child_path = temp_dir.join("worktrees").join("parent").join("feature-b");
        ...
        let child = WorkspaceEntry {
            id: "wt-remove-git-failed".to_string(),
            name: "feature-b".to_string(),
            path: child_path.to_string_lossy().to_string(),
            kind: WorkspaceKind::Worktree,
            parent_id: Some(parent.id.clone()),
