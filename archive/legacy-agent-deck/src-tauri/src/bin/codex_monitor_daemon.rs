...
#[path = "../storage.rs"]
mod storage;
#[path = "agent_deck_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "agent-deck-daemon";
...
impl DaemonState {
    ...
    async fn remove_workspace(&self, id: String) -> Result<(), String> {
        workspaces_core::remove_workspace_core(
            ...
            workspaces_core::remove_worktree_dir_all,
            true,
            true,
        )
        .await
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("agent-deck-daemon");
        }
    }
    ...
    PathBuf::from(home)
        .join(".local")
        .join("share")
        .join("agent-deck-daemon")
}

fn usage() -> String {
    format!(
        "\
USAGE:\n  agent-deck-daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n\n\
...
mod tests {
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            codex_login_cancels: Mutex::new(HashMap::new()),
            daemon_binary_path: Some("/tmp/agent-deck-daemon".to_string()),
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "agent-deck-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
