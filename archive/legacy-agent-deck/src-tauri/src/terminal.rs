...
fn terminal_key_matches_workspace(key: &str, workspace_id: &str) -> bool {
    key.strip_prefix(workspace_id)
...
pub(crate) async fn close_workspace_terminal_sessions(state: &AppState, workspace_ids: &[String]) {
    ...
    let sessions = {
        ...
        let keys = sessions
            ...
            .filter(|key| {
                workspace_ids
                    .iter()
                    .any(|workspace_id| terminal_key_matches_workspace(key, workspace_id))
            })
            .cloned()
            .collect::<Vec<_>>();
...
#[cfg(test)]
mod tests {
    use super::{terminal_key_matches_workspace, unix_shell_args, windows_shell_args};

    #[test]
    fn terminal_key_workspace_match_requires_key_separator() {
        assert!(terminal_key_matches_workspace(
            "workspace-1:main",
            "workspace-1"
        ));
        assert!(!terminal_key_matches_workspace(
