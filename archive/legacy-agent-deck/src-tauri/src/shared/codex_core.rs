...
fn temp_converted_image_path(path: &str, extension: &str) -> PathBuf {
    ...
    let ts = SystemTime::now()
        ...
        .unwrap_or_default();
    std::env::temp_dir().join(format!("agent-deck-image-{safe_stem}-{ts}.{extension}"))
}
...
pub(crate) async fn skills_list_core(
    ...
    // Some environments don't surface those reliably in AgentDeck unless we
...
mod tests {
    ...
    fn read_image_data_url_core_succeeds_with_file_uri_for_real_file() {
        let dir = std::env::temp_dir().join("agent_deck_test");
