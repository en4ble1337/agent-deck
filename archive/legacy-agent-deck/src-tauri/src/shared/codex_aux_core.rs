...
fn extract_device_login_fields(output: &str) -> (Option<String>, Option<String>) {
    ...
    for token in cleaned.split_whitespace() {
        ...
        if auth_url.is_none() && (token.starts_with("https://") || token.starts_with("http://")) {
        ...
        {
            user_code = Some(token.to_string());
        }
    }
    (auth_url, user_code)
}

fn spawn_device_auth_line_reader<R>(stream: R, tx: mpsc::UnboundedSender<String>)
where
...
fn is_logged_in_status_output(output: &str) -> bool {
    let lower = strip_ansi_codes(output).to_ascii_lowercase();
    lower.contains("logged in") && !lower.contains("not logged in")
}

async fn codex_login_status_output(
    codex_bin: Option<String>,
    ...
    let output = match timeout(Duration::from_secs(10), command.output()).await {
        Ok(result) => result.map_err(|err| format!("Failed to check Codex login status: {err}"))?,
...
pub(crate) async fn codex_device_login_core(
    app_settings: &Mutex<AppSettings>,
) -> Result<Value, String> {
    let (default_bin, default_args) = {
        let settings = app_settings.lock().await;
    ...
    if let Some(status_output) =
        codex_login_status_output(default_bin.clone(), default_args.as_deref()).await?
    {
        if is_logged_in_status_output(&status_output) {
            return Ok(json!({
                "loginId": "codex-already-authorized",
                "authUrl": "",
                "userCode": "",
                "alreadyAuthenticated": true,
                "status": strip_ansi_codes(&status_output).trim(),
            }));
        }
    }
    ...
    let mut child = command
        .spawn()
        .map_err(|err| format!("Failed to start Codex device auth: {err}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "Failed to capture Codex device auth stdout".to_string())?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| "Failed to capture Codex device auth stderr".to_string())?;

    let (tx, mut rx) = mpsc::unbounded_channel::<String>();
    spawn_device_auth_line_reader(stdout, tx.clone());
    spawn_device_auth_line_reader(stderr, tx.clone());
    drop(tx);
    ...
    let started = timeout(Duration::from_secs(30), async {
        while let Some(line) = rx.recv().await {
            ...
            output.push('\n');
            let (next_url, next_code) = extract_device_login_fields(&output);
            if auth_url.is_none() {
                auth_url = next_url;
            }
            if user_code.is_none() {
        ...
        let cleaned_output = strip_ansi_codes(&output).trim().to_string();
        if cleaned_output.is_empty() {
            Err("Codex device auth exited before printing a login code.".to_string())
        } else {
            Err(format!(
                "Codex device auth exited before printing a login code: {cleaned_output}"
            ))
        }
    })
    .await
    .map_err(|_| "Timed out waiting for Codex device auth code.".to_string())?;
    ...
    let login_id = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|duration| duration.as_millis())
        .unwrap_or_default();

    Ok(json!({
        "loginId": format!("codex-device-{login_id}"),
        "authUrl": auth_url.unwrap_or_default(),
        "userCode": user_code.unwrap_or_default(),
    }))
}
